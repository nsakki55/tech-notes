...
### 5-5-6
畳み込みニューラルネットワークについて。局所的特徴を抽出している。 
局所的受容野：全ての入力の一部分のみを入力とする。  
重み共有：畳み込み層における各ユニットの重みを共有することで、モデルの複雑さに制限をかける  
部分サンプリング：Pooling することで、不変性を獲得  
畳み込み層の役割  
→局所的特徴量抽出に相当。何種類ものフィルタをかけることで、様々な特徴を得ることが期待される。  
Pooling層の役割  
→位置に対して敏感にする効果がある。  
構造全体：畳み込み層とプーリング層をあわせることで、だいたいこのあたりに局所的特徴があるということが表現される。  

### 5-5-7
ソフト重み共有。基本的なCNNは畳み込み層の重みを共有するハードな制約  
各重みの事前分布として混合ガウス分布を仮定して、事後確率の最大化（MAP推定）  
開放はEMアルゴリズムを用いる。  
